...
// Ensure date helpers exist on window (protect against cached/duplicate scripts)
...
let totalAnswers = 0;
let startTime = null;
let startTimeString = ''; // Time when quiz started (HH:MM format)
...
let hasCompletedSessionToday = false; // Whether user has completed at least one quiz session today
...
const dailyCompleteBtn = document.getElementById('daily-complete-btn');
...
const dailyCompleteUsernameEl = document.getElementById('daily-complete-username');
const levelupUsernameEl = document.getElementById('levelup-username');
const levelupTextEl = document.getElementById('levelup-text');
const quizDailyLabel = document.getElementById('quiz-daily-label');
const resultsDailyLabel = document.getElementById('results-daily-label');
...
function init() {
    ...
    if (dailyCompleteBtn) {
        dailyCompleteBtn.addEventListener('click', () => showScreen('start'));
    }
...
function getTodayDateString() {
    // Use toLocaleDateString with en-CA to get YYYY-MM-DD in local timezone
    try {
        const localIso = new Date().toLocaleDateString('en-CA');
        if (localIso && typeof localIso === 'string') return localIso;
    } catch (e) {
...
// Format a Date object to local YYYY-MM-DD (avoid toISOString which returns UTC)
...
// Ensure these helpers are available on the global `window` object
if (typeof window !== 'undefined') {
    try {
...
function loadTodayScore() {
    ...
    const sessionKey = `session_${username}_${today}`;
    // Ensure today's keys exist (avoid missing-key issues on first load)
    ...
    hasCompletedSessionToday = savedSession === 'true';
    
    // Check if it's a new day and reset session flag if needed
    const lastSessionDate = localStorage.getItem(`lastSessionDate_${username}`);
    if (lastSessionDate !== today) {
        hasCompletedSessionToday = false;
        // Do not overwrite lastSessionDate here; it should be set when a session is actually completed
    }
}
...
// Mark that user has completed a session today
function markSessionCompleted() {
    const today = getTodayDateString();
    const sessionKey = `session_${username}_${today}`;
    localStorage.setItem(sessionKey, 'true');
    hasCompletedSessionToday = true;
    // Record the date of the last completed session
    localStorage.setItem(`lastSessionDate_${username}`, today);
}
...
function getScoreHistory(days = 7) {
    ...
    for (let i = days - 1; i >= 0; i--) {
        ...
        const dailyAnswers = score ? parseInt(score) : 0;
        ...
        history.push({
            ...
            score: dailyAnswers  // Use daily count instead of cumulative
        });
    }
...
const commentData = [
    { category: 1, comment: '今日もいつものペースで進められていますね。落ち着いた良いペースです。' },
    { category: 1, comment: '日々の学習量が安定していて、とても安心感のある続け方ですね。' },
    { category: 1, comment: '無理のないペースがしっかり定着してきています。このリズム、大切にしましょう。' },
    { category: 1, comment: 'バランスのとれた学習が続いています。心にもゆとりがありそうですね。' },
    { category: 1, comment: '「ちょうどいい量」を保てているのが素敵です。自分に合った習慣になっています。' },
    { category: 1, comment: '今日も安定した学習量。コツコツ型の強みが生きていますね。' },
    { category: 1, comment: '大きな上下が少ない、安心できるグラフになっていますね。とても堅実な続け方です。' },
    { category: 1, comment: '積み重ねが"なめらかな線"になってきています。この安定感は大きな財産です。' },
    { category: 1, comment: 'マイペースな継続ができていますね。心も体も無理していない良い状態です。' },
    { category: 1, comment: '学習量の波が小さくて、毎日が整ったリズムになっています。その調子です。' },
    { category: 2, comment: 'いつもより少し多めの学習ですね。バランスを保ちながら、良いアクセントになっています。' },
    { category: 2, comment: '今日は少しだけギアを上げられましたね。無理のない範囲での前進、すばらしいです。' },
    { category: 2, comment: 'ほんの少し増えただけでも、積み重なると大きな自信につながりますね。' },
    { category: 2, comment: 'ベースは安定、その上に少しプラス。とても安心感のある伸ばし方です。' },
    { category: 2, comment: '自分のペースを守りながら、ちょっとだけチャレンジできましたね。ナイスバランスです。' },
    { category: 2, comment: '学習量が緩やかに増えています。焦りではなく、落ち着いた向上心を感じます。' },
    { category: 2, comment: '小さな"プラス"が、心地よい達成感につながっていますね。' },
    { category: 2, comment: '今日の増加は、明日以降の安定したペースづくりにも役立ちそうです。' },
    { category: 3, comment: '今日は少しペースを落としていますね。休みながら続けられているのがとても良いです。' },
    { category: 3, comment: '学習量が減った日も、グラフがつながっていること自体が立派な継続です。' },
    { category: 3, comment: 'その日の体調や気持ちに合わせて調整できているのが、長く続く人の特徴です。' },
    { category: 3, comment: '少なめの日があることで、全体のバランスが整います。安心してマイペースでいきましょう。' },
    { category: 3, comment: '「今日はこのくらいで大丈夫」と線を引けるのも、とても大切な力です。' },
    { category: 3, comment: '減った日も含めて、あなたのリズムです。全体として安定していれば問題ありません。' },
    { category: 3, comment: 'ほんの少しでも単語に触れられていること自体が、習慣の維持につながっています。' },
    { category: 3, comment: 'がんばりすぎない日があるからこそ、明日も続けられます。良いペース調整ですね。' },
    { category: 4, comment: '間があいても、また戻ってこられたことが一番の"安定力"です。おかえりなさい。' },
    { category: 4, comment: '再開できたこと自体が、強い習慣の土台になっています。とても大切な一歩です。' },
    { category: 4, comment: 'グラフが再び動き出しましたね。止まっても、また動けば大丈夫です。' },
    { category: 4, comment: '一度リズムを崩しても、落ち着いて戻せるのは、バランス感覚がある証拠です。' },
    { category: 4, comment: 'ペースを取り戻そうとしている姿勢が、長期的な安定につながっていきます。' },
    { category: 4, comment: '「また少しずつ始めよう」と思えた、その気持ちが何より大切です。' }
];

function generateComment() {
    const history = getScoreHistory(7);
    const todayData = history[history.length - 1];
    const yesterdayData = history.length > 1 ? history[history.length - 2] : null;

    let category = 1; // default to stable

    if (!yesterdayData || yesterdayData.score === 0) {
        // 再開した日(前日が0問)
        category = 4;
    } else {
        const todayScore = todayData.score;
        const yesterdayScore = yesterdayData.score;
    ...
    const categoryComments = commentData.filter(item => item.category === category);

    if (categoryComments.length === 0) {
        return "今日も学習お疲れ様です。継続して頑張りましょう！";
    }

    // Select random comment from category
    const randomIndex = Math.floor(Math.random() * categoryComments.length);
    return categoryComments[randomIndex].comment;
}
...
function getFirstUsedDate() {
    ...
    if (!firstUsedDate) {
        // Set today (local) as first used date
...
function ensureAllDaysRecorded() {
    ...
    // Parse stored YYYY-MM-DD into a local Date to avoid timezone shift
    const parts = firstUsedDate.split('-');
...
function showScreen(screen) {
    ...
    switch (screen) {
        ...
        case 'quiz':
            ...
            // Show label only if today's answers >= 1 AND this is not the first session
            if (todayTotalAnswers >= 1 && hasCompletedSessionToday) {
                quizDailyLabel.classList.remove('hidden');
            } else {
                quizDailyLabel.classList.add('hidden');
            }
            break;
        case 'results':
            ...
            // Show label only if today's answers >= 1 AND this is not the first session
            if (todayTotalAnswers >= 1 && hasCompletedSessionToday) {
                resultsDailyLabel.classList.remove('hidden');
            } else {
                resultsDailyLabel.classList.add('hidden');
            }
            break;
        case 'daily-complete':
            dailyCompleteScreen.classList.remove('hidden');
            dailyCompleteUsernameEl.textContent = username + 'さん';
            break;
        case 'levelup':
...
function startQuiz() {
    ...
    // Record start time in HH:MM format
    const now = new Date();
...
function showResults() {
    ...
    // Display start time
    document.getElementById('start-time').textContent = startTimeString || '--:--';
    ...
    // Set button text based on next screen
    // If daily task completion screen will be shown (1st session), show "次へ"
    // Otherwise (2nd+ session), show "終了する"
    if (!hasCompletedSessionToday && todayTotalAnswers >= 1) {
...
function drawProgressChart() {
    ...
    // Get all days from first use to today
    const firstUsedDate = getFirstUsedDate();
    // Parse stored YYYY-MM-DD into a local Date to avoid timezone shift
    const parts = firstUsedDate.split('-');
    const firstDate = new Date(Number(parts[0]), Number(parts[1]) - 1, Number(parts[2]));
    ...
    // Display all data from first use to today
    const displayData = allHistory;

    const labels = displayData.map(item => item.displayDate);
    const data = displayData.map(item => item.score);

    // Calculate max value to determine step size
    ...
    progressChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: data,
                borderColor: '#3ba5d4',
                backgroundColor: 'rgba(59, 165, 212, 0.1)',
                borderWidth: 3,
                tension: 0.3,
                pointRadius: 5,
                pointBackgroundColor: '#3ba5d4',
        ...
        plugins: [{
            afterDraw: function (chart) {
                ...
                // Draw (問) above the chart area, right-aligned with y-axis ticks
                ctx.textAlign = 'right';
                ctx.textBaseline = 'bottom';
                ctx.fillText('(問)', yAxis.left - 5, chartArea.top - 5);

                // Draw (日) to the right of the chart area, top-aligned with x-axis
                ctx.textAlign = 'left';
                ctx.textBaseline = 'top';
                ctx.fillText('(日)', chartArea.right + 10, xAxis.bottom);
...
function handleFinish() {
    // Check if daily task is complete (1+ answers today) AND this is the first session
    if (!hasCompletedSessionToday && todayTotalAnswers >= 1) {
