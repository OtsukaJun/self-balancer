...
function drawProgressChart() {
    ...
    // Filter to show only days with scores or today
    const today = getTodayDateString();
    const filteredHistory = history.filter(item =>
        item.score > 0 || item.date === today
    );

    // If less than 2 data points, show last 3 days
    const displayData = filteredHistory.length >= 2 ? filteredHistory : history.slice(-3);
